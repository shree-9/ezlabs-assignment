    ...
    <section className="relative min-h-screen bg-[rgba(255,157,125,0.7)] flex flex-col justify-center items-center text-[#333] px-10">
      ...
      <div className="flex items-center justify-center gap-12 mt-8 flex-wrap">
        <div className="shadow-xl bg-white p-2 rounded-md">
          ...
          <p className="text-center mt-2 text-sm">Art Curation</p>
        </div>
      ...
      <div className="flex justify-between w-full max-w-5xl mt-10">
        <button onClick={() => navigate("/branding")} className="service-btn">&lt; Back</button>
        <button onClick={() => navigate("/services")} className="service-btn">Explore Now →</button>
      </div>

      <img src={frameIcon} alt="" className="absolute top-20 right-12 w-10 opacity-80" />
      <img src={paletteIcon} alt="" className="absolute bottom-16 left-10 w-10 opacity-80" />
    </section>
  );
}
